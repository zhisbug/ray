...
def test_allreduce_multiple_call(ray_start_distributed_2_nodes_4_gpus, num_calls):
    world_size = 3
    actors, _ = create_collective_workers(world_size)
...
def test_allreduce_multiple_group_call(ray_start_distributed_2_nodes_4_gpus, num_groups, num_calls):
    world_size = 3
    actors, _ = create_collective_workers(world_size)
    for group_name in range(1, num_groups):
        ray.get([
